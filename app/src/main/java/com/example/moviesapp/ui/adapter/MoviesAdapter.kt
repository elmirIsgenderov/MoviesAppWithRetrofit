...
class MoviesAdapter(private var context: Context, private var moviesList: List<Movies>) :
    ...
    override fun onBindViewHolder(holder: cardHolder, position: Int) {
        ...
        val url = "http://kasimadalan.pe.hu/filmler_yeni/resimler/${movie.picture}"
        Glide.with(context).load(url).override(500, 750).into(holder1.imgFilm)

        holder1.txtPrice.text = "${movie.price}"
        ...
        holder1.movieCard.setOnClickListener {
            val transition = MainFragmentDirections.transitionDetail(movie)
